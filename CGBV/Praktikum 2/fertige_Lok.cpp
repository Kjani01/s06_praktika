...
void CreateGeometry()
{
	...
	float radius = 50.0;
	float constHeight = 10.0 / tesselation; //Anfang. 11 Ringe mit 10x Abstand
	float height = 10.0 / tesselation;
	...
	auto sphereVertices = new M3DVector3f[arr];
	auto sphereColors = new M3DVector4f[arr];
	float sphereMax = 100;
	...
	for (int k = 1; k <= 2 + 16 * tesselation; k++) {
		float actualRadius = sqrt((pow(radius, 2) - (pow((radius - height), 2))));
		float x = actualRadius * sin((GL_PI / (8.0f * tesselation)) * k);
		float y = actualRadius * cos((GL_PI / (8.0f * tesselation)) * k);
		float x1 = actualRadius * sin((GL_PI / (8.0f * tesselation)) * (k + 1));
		float y1 = actualRadius * cos((GL_PI / (8.0f * tesselation)) * (k + 1));

		m3dLoadVector3(sphereVertices[i], 0, 0, sphereMax);
		m3dLoadVector3(sphereVertices[i + 1], x, y, (sphereMax - height));

		m3dLoadVector3(sphereVertices[i + 2], 0, 0, sphereMax);
		m3dLoadVector3(sphereVertices[i + 3], x1, y1, (sphereMax - height));

		//Farben setzen
		if ((k % 2) == 0) {
			m3dLoadVector4(sphereColors[i], 0.396, 0.810, 0.810, 1);
			m3dLoadVector4(sphereColors[i + 1], 0.396, 0.810, 0.810, 1);
			m3dLoadVector4(sphereColors[i + 2], 0.396, 0.810, 0.810, 1);
			m3dLoadVector4(sphereColors[i + 3], 0.396, 0.810, 0.810, 1);
		}
		else {
			m3dLoadVector4(sphereColors[i], 0.635, 0.235, 0.635, 1);
			m3dLoadVector4(sphereColors[i + 1], 0.635, 0.235, 0.635, 1);
			m3dLoadVector4(sphereColors[i + 2], 0.635, 0.235, 0.635, 1);
			m3dLoadVector4(sphereColors[i + 3], 0.635, 0.235, 0.635, 1);
		}
	...
	for (int l = 1; l <= 8 * tesselation + 2 * (tesselation - 1); l++) {
		for (int k = 1; k <= 2 + 16 * tesselation; k++) {
			...
			float newRadius = sqrt((pow(radius, 2) - (pow((radius - (height + (10.0 / tesselation))), 2))));
			float xNew = newRadius * sin((GL_PI / (8.0f * tesselation)) * k);
			float yNew = newRadius * cos((GL_PI / (8.0f * tesselation)) * k);
			float x1New = newRadius * sin((GL_PI / (8.0f * tesselation)) * (k + 1));
			float y1New = newRadius * cos((GL_PI / (8.0f * tesselation)) * (k + 1));

			float newHeight1 = (sphereMax - (constHeight * l));
			float newHeight2 = (sphereMax - (constHeight * (l + 1)));

			m3dLoadVector3(sphereVertices[i], x, y, newHeight1); // sphereHeight[l]
			m3dLoadVector3(sphereVertices[i + 1], xNew, yNew, newHeight2); // sphereHeight[l + 1]
			m3dLoadVector3(sphereVertices[i + 2], x1, y1, newHeight1);
			m3dLoadVector3(sphereVertices[i + 3], x1New, y1New, newHeight2);
	...
	for (int k = 1; k <= 2 + 16 * tesselation; k++) {
		...
		m3dLoadVector3(sphereVertices[i], x, y, constHeight); //
		m3dLoadVector3(sphereVertices[i + 1], 0, 0, 0);

		m3dLoadVector3(sphereVertices[i + 2], x1, y1, constHeight); //sphereHeight[8]
		m3dLoadVector3(sphereVertices[i + 3], 0, 0, 0);

		//Farben setzen 
		if ((k % 2) == 0) {
			m3dLoadVector4(sphereColors[i], 0.396, 1, 0.810, 1);
			m3dLoadVector4(sphereColors[i + 1], 0.396, 0.810, 0.810, 1);
			m3dLoadVector4(sphereColors[i + 2], 0.396, 1, 0.810, 1);
			m3dLoadVector4(sphereColors[i + 3], 0.396, 0.810, 0.810, 1);
		}
		else {
			m3dLoadVector4(sphereColors[i], 0.635, 0.235, 0.635, 1);
	...
	sphere.CopyVertexData3f(sphereVertices);
	sphere.CopyColorData4f(sphereColors);
	sphere.End();
