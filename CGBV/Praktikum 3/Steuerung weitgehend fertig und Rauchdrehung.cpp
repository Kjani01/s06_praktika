...
{
	if (rotateGier) {
		if (x > start) { //mehr links sehen
			rotateX[0][0] = rotateX[0][0] + 5;
			rotateX[0][2] = rotateX[0][2] + 5;
			start = x;
		}
		else  {		//mehr rechts sehen
			rotateX[0][0] = rotateX[0][0] - 5;
			rotateX[0][2] = rotateX[0][2] - 5;
			start = x;
		}
	...
	int y)      /* I - Current mouse Y position */
{
	...
	if (state == GLUT_DOWN) {
		...
		/*if (button == 0) {
			...
			if (stop < start) {
				right = true;
				printf("drehen von rechts nach links also wollen wir mehr rechts sehen\n");
			}
			else {
				left = true;
				printf("drehen von links nach rechts also wollen wir mehr links sehen\n");
			}
		}
		else if (button == 2) {
			//printf("Rotate stop\n");
			...
			if (stop < start) {
				down = true;
				printf("drehen von unten nach oben also wollen wir mehr unten sehen \n");
			}
			else {
				up = true;
				printf("drehen von oben nach unten also wollen wir mehr oben sehen\n");
			}
		}*/
	}
