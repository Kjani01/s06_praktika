...
void CreateGeometry()
{
	...
	for (i = 1; i < (4 * tesselierung + 1); i++) {
		normalize[lauf2][0] = (bodenVertices[i - 1][0] * 2.0 - bodenVertices[i - 1][0]) / 2;
		normalize[lauf2][1] = (bodenVertices[i - 1][1] * 2.0 - bodenVertices[i - 1][1]) / 2;
		normalize[lauf2][2] = 0;

		//normalize[0][ = normalize[0]{ xV, yV, zV };
		m3dNormalizeVector3(normalize[lauf2]);
		geometryBatch.Normal3f(normalize[lauf2][0], normalize[lauf2][1], normalize[lauf2][2]);
		...
		if (i < (4 * tesselierung)) {
			...
			normalize[lauf2][0] = bodenVertices[i - 1][0] - bodenVertices[i - 1][0] * 2.0 ;
			normalize[lauf2][1] = bodenVertices[i - 1][1] - bodenVertices[i - 1][1] * 2.0 ;
			normalize[lauf2][2] = 0;
			m3dNormalizeVector3(normalize[lauf2]);
			geometryBatch.Normal3f(normalize[lauf2][0], normalize[lauf2][1], normalize[lauf2][2]);
			lauf2--;
	...
	normalize[lauf2][0] = bodenVertices[0][0] - bodenVertices[0][0] * 2.0 ;
	normalize[lauf2][1] = bodenVertices[0][1] - bodenVertices[0][1] * 2.0;
	normalize[lauf2][2] = 0;
	...
	*/
	...
	for (i = 0; i < (4 * tesselierung ); i++) {
		...
		if (i == 0) {
			//passt
			geometryBatch.Normal3f(normalize[lauf + i + 14][0], normalize[lauf + i + 14][1], normalize[lauf + i + 13][2]);
			...
			//TODO
			//geometryBatch.Normal3f(normalize[lauf + i - 1][0], normalize[lauf + i - 1][1], normalize[lauf + i - 1][2]);
			geometryBatch.Normal3f(normalize[lauf + i][0], normalize[lauf + i][1], normalize[lauf + i][2]);
			...
			if (lauf < 7) {
				...
				//geometryBatch.Normal3f(0.0, 0.0, 0.5);
				geometryBatch.Normal3f(normalize[lauf + i + 1 + 1][0], normalize[lauf + i + 1 + 1][1], normalize[lauf + i + 1 + 1][2]);
				geometryBatch.Vertex3f(boden2Vertices[(i + 1) % (4 * tesselierung)][0], boden2Vertices[(i + 1) % (4 * tesselierung)][1], boden2Vertices[(i + 1) % (4 * tesselierung)][2]);
				normVecs.Vertex3f(boden2Vertices[(i + 1) % (4 * tesselierung)][0], boden2Vertices[(i + 1) % (4 * tesselierung)][1], boden2Vertices[(i + 1) % (4 * tesselierung)][2]);
				normVecs.Vertex3f(boden2Vertices[(i + 1) % (4 * tesselierung)][0], boden2Vertices[(i + 1) % (4 * tesselierung)][1], boden2Vertices[(i + 1) % (4 * tesselierung)][2] + 0.5);
				lauf++;
	...
	//Deckel - these normalvectors are working!!!
	for (i = 1; i < (4 * tesselierung + 1); i++) {
		/*normalize[lauf][0] = bodenVertices[i - 1][0] * 2.0 - bodenVertices[i - 1][0];
		normalize[lauf][1] = bodenVertices[i - 1][1] * 2.0 - bodenVertices[i - 1][1];
		normalize[lauf][2] = 0.25;
		m3dNormalizeVector3(normalize[lauf]);*/
		geometryBatch.Normal3f(normalize[lauf][0], normalize[lauf][1], normalize[lauf][2]);
		lauf++;
		...
		if (i < (4 * tesselierung)) {
			//printf("Writing only in 4 cases\n");
			/*normalize[lauf][0] = bodenVertices[i][0] * 2.0 - bodenVertices[i][0];
			normalize[lauf][1] = bodenVertices[i][1] * 2.0 - bodenVertices[i][1];
			normalize[lauf][2] = 0.25;
			m3dNormalizeVector3(normalize[lauf]);*/
			geometryBatch.Normal3f(normalize[lauf][0], normalize[lauf][1], normalize[lauf][2]);
			lauf++;
	...
	/*normalize[lauf][0] = bodenVertices[0][0] * 2.0 - bodenVertices[0][0];
	normalize[lauf][1] = bodenVertices[0][1] * 2.0 - bodenVertices[0][1];
	normalize[lauf][2] = 0.25;
	m3dNormalizeVector3(normalize[lauf]);*/
	geometryBatch.Normal3f(normalize[lauf][0], normalize[lauf][1], normalize[lauf][2]);
	lauf++;
