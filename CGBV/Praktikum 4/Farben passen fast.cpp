...
void CreateGeometry()
{
	...
	for (i = 1; i < (4 * tesselierung + 1); i++) {
		normalize[lauf][0] = (bodenVertices[i - 1][0] * 2.0 - bodenVertices[i - 1][0]) / 2;
		normalize[lauf][1] = (bodenVertices[i - 1][1] * 2.0 - bodenVertices[i - 1][1]) / 2;
		normalize[lauf][2] = 0;
		...
		if (i < (4 * tesselierung)) {
			...
			normalize[lauf][0] = bodenVertices[i - 1][0] - bodenVertices[i - 1][0] * 2.0 ;
			normalize[lauf][1] = bodenVertices[i - 1][1] - bodenVertices[i - 1][1] * 2.0 ;
			normalize[lauf][2] = 0;
			m3dNormalizeVector3(normalize[lauf]);
			geometryBatch.Normal3f(normalize[lauf][0], normalize[lauf][1], normalize[lauf][2]);
			lauf--;
	...
	normalize[lauf][0] = bodenVertices[0][0] - bodenVertices[0][0] * 2.0 ;
	normalize[lauf][1] = bodenVertices[0][1] - bodenVertices[0][1] * 2.0;
	normalize[lauf][2] = 0;
	...
	//Deckel - this normalvectors are working!!!
	for (i = 1; i < (4 * tesselierung + 1); i++) {
	...
	for (i = 1; i < (4 * tesselierung + 1); i++) {
		...
		geometryBatch.Color4f(0.235, 0.235, 0.635, 1);
		...
		//geometryBatch.Normal3f(0.0, 0.0, 0.5); 
		geometryBatch.Normal3f(normalize[lauf + i + 8][0], normalize[lauf + i + 8][1], normalize[lauf + i + 8][2]);
		...
		//geometryBatch.Normal3f(0.0, 0.0, 0.5);
		geometryBatch.Normal3f(normalize[lauf + i + 1 + 8][0], normalize[lauf + i + 1 + 8][1], normalize[lauf + i + 1 + 8][2]);
